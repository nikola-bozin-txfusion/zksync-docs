...
const greeterContract = await deployer.deploy(mainContractArtifact); // No constructor arguments.
```
...
Next step is to execute the deploy command:
...
const initializerFunctionName = 'initialize'
const boxFactory = await hre.ethers.getContractFactory("Box");
const box = await hre.zkUpgrades.deployProxy(boxArtifact, constructorArguments, { initializer: initializerFunctionName }, hre.deployer.zkWallet);
await mainContract.waitForDeployment();
```
...
let upgrade = await hre.upgrades.upgradeProxy(upgradableProxyAddress,boxV2ContractFactory);
```
...
const upgradableProxyAddress = "UPGRADEABLE_PROXY_ADDRESS";
const boxV2ContractFactory = await hre.ethers.getContractFactory("BoxV2");
const boxV2 = await hre.zkUpgrades.upgradeProxy(upgradableProxyAddress,boxV2ContractFactory,hre.deployer.zkWallet);
```
...
const initializerFunctionName = 'initialize'
const boxFactory = await hre.ethers.getContractFactory("Box");
const box = await hre.zkUpgrades.deployBeacon(boxFactory, constructorArguments, { initializer: initializerFunctionName }, hre.deployer.zkWallet);
await mainContract.waitForDeployment();
```
